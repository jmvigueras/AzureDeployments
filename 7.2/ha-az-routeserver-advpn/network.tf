...
resource "azurerm_virtual_network" "vnet-spokec" {
  name                = "${var.prefix}-vnet-spokec"
  address_space       = [var.spokeccidr]
  location            = var.location
  resource_group_name = azurerm_resource_group.myterraformgroup.name
...
resource "azurerm_subnet" "subnet-spokec" {
  name                 = "${var.prefix}-subnet-spokec"
  resource_group_name  = azurerm_resource_group.myterraformgroup.name
  virtual_network_name = azurerm_virtual_network.vnet-spokec.name
  address_prefixes     = [var.spokecsubnetcidr]
}

resource "azurerm_subnet" "subnet-spokec-routeserver" {
  name                 = "RouteServerSubnet"
  resource_group_name  = azurerm_resource_group.myterraformgroup.name
  virtual_network_name = azurerm_virtual_network.vnet-spokec.name
  address_prefixes     = [var.spokecsubnetrouteserver]
}
...
resource "azurerm_subnet" "subnet-advpn" {
  ...
  resource_group_name  = azurerm_resource_group.myterraformgroup.name
  virtual_network_name = azurerm_virtual_network.vnet-fgt.name
  address_prefixes     = [var.advpncidr]
}
...
resource "azurerm_public_ip" "cluster-advpn-ip" {
  name                = "${var.prefix}-cluster-advpn-ip"
  location            = var.location
  resource_group_name = azurerm_resource_group.myterraformgroup.name
  allocation_method   = "Static"
...
resource "azurerm_network_interface" "ni-activeport4" {
  ...
  ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.cluster-advpn-ip.id
  }
...
resource "azurerm_network_interface_security_group_association" "port4nsg" {
  depends_on                = [azurerm_network_interface.ni-activeport4]
  network_interface_id      = azurerm_network_interface.ni-activeport4.id
  network_security_group_id = azurerm_network_security_group.nsg-public.id
}
...
resource "azurerm_network_interface_security_group_association" "passiveport4nsg" {
  depends_on                = [azurerm_network_interface.ni-passiveport4]
  network_interface_id      = azurerm_network_interface.ni-passiveport4.id
  network_security_group_id = azurerm_network_security_group.nsg-public.id
}
...
resource "azurerm_virtual_network_peering" "peerSpokeCtoFGT-1" {
  name                      = "${var.prefix}-peerSpokeCtoFGT-1"
  resource_group_name       = azurerm_resource_group.myterraformgroup.name
  virtual_network_name      = azurerm_virtual_network.vnet-fgt.name
  remote_virtual_network_id = azurerm_virtual_network.vnet-spokec.id
}

resource "azurerm_virtual_network_peering" "peerSpokeCtoFGT-2" {
  name                      = "${var.prefix}-peerSpokeCtoFGT-2"
  resource_group_name       = azurerm_resource_group.myterraformgroup.name
  virtual_network_name      = azurerm_virtual_network.vnet-spokec.name
  remote_virtual_network_id = azurerm_virtual_network.vnet-fgt.id
  allow_forwarded_traffic   = true
